...
type PluginUpdateCheck = {
  ...
  updates: Array<{
    ...
    installBlocker?: 'local-mapping-conflict' | 'identity-casing-conflict' | 'invalid-target-state'
  ...
  <div class="app-shell">
    ...
    <main class="workspace">
      ...
      <section v-show="activeSection === 'overview'" class="page page-overview" aria-labelledby="overview-title">
        ...
        <section v-if="controlLoadFailed || controlRefreshIncomplete || runtimeStatusStale || mappingWorkspaceUnverified || managedProcessRestartRequired || trackedInvocationsUnavailable || projectDeliveryDraftDirty || needsDeepDeploymentCheck || deploymentCheck?.failures || status?.businessTimedOutWindows || status?.pluginPreflightFailures || status?.pluginApiBaselineFailures || status?.pluginHosts.some(pluginHostNeedsAttention) || inventory?.quarantined.length || ssoError" class="attention-panel">
          ...
          <ul>
            ...
            <li v-if="status?.pluginApiBaselineFailures"><strong>插件能力基线有 {{ status.pluginApiBaselineFailures }} 次持久化失败</strong><button type="button" @click="activeSection = 'security'">查看诊断</button></li>
      ...
      <section v-show="activeSection === 'configuration' || activeSection === 'delivery'" class="page" :aria-labelledby="activeSection === 'delivery' ? 'delivery-title' : 'configuration-title'">
        ...
        <header v-show="activeSection === 'delivery'" class="section-header"><div><p class="eyebrow">PROJECT DELIVERY</p><h1 id="delivery-title">项目交付</h1><p>按源机准备、目标机导入和 Windows 实机验证完成可重复交付。</p></div><div class="header-actions"><button class="primary" type="button" :disabled="busy || projectStateUnverified || projectDeliveryDraftDirty || deploymentCheck?.deepAvailable === false" @click="runDeploymentCheck">{{ deploymentCheck?.deepAvailable === false ? '仅 Windows 可深度自检' : '深度自检' }}</button><button type="button" :disabled="busy || projectStateUnverified || projectDeliveryDraftDirty || deploymentCheck?.deepAvailable === false" @click="exportDeploymentCheck">导出深度自检记录</button></div></header>
        <ol v-show="activeSection === 'delivery'" class="delivery-steps" aria-label="项目交付步骤">
          <li><b>01</b><span><strong>源机导出交付草稿</strong><small>冻结当前配置、签名插件和本地映射，生成待组织签名的项目包。</small></span></li>
          <li><b>02</b><span><strong>目标机导入已签项目包</strong><small>选择项目包及同目录组织旁签，核对变更计划后原子切换项目。</small></span></li>
          <li><b>03</b><span><strong>目标机完成深度验证</strong><small>打开真实业务页面，在 Windows 上验证 x86/x64 宿主并导出现场记录。</small></span></li>
        </ol>
      ...
      <section v-show="activeSection === 'security'" class="page" aria-labelledby="security-title">
        ...
        <section :class="['diagnostic-grid', { stale: controlRefreshIncomplete || runtimeStatusStale }]" aria-label="详细运行状态">
          ...
          <article><span>插件与映射信任</span><strong>{{ status?.pluginTrustMode === 'ed25519-strict' ? '严格签名' : '开发模式' }}</strong><small :title="status?.pluginRoot">{{ status ? `${status.trustKeyCount} 把密钥 · ${status.pluginApiBaselineCount} 个能力基线 · 基线写入失败 ${status.pluginApiBaselineFailures}` : '完整清单与 SHA-256 校验' }}</small></article>
