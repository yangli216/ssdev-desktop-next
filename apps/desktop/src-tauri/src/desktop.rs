...
mod tests {
    ...
    fn business_bridge_matches_the_shared_sdk_contract() {
        ...
        assert_eq!(
            ...
            crate::TRACKED_INVOCATION_ERROR_SCHEMA_VERSION
        );
        assert_eq!(
            contract["trackedInvocationError"]["kind"],
            "trackedInvocationError"
        );
        assert_eq!(
            contract["trackedInvocationError"]["phases"],
            serde_json::json!(crate::TRACKED_INVOCATION_ERROR_PHASES)
        );
        assert_eq!(
            contract["trackedInvocationStatus"]["states"],
            serde_json::json!(crate::TRACKED_INVOCATION_STATUS_STATES)
        );
        assert_eq!(
