...
use webplus_plugin_config::{
    build_local_mapping_integrity, MethodDefinition, ParameterDefinition, PluginManifest,
    PluginMetadata, ServiceDefinition, API_FILENAME, LOCAL_MAPPING_INTEGRITY_FILENAME,
...
fn generate_typescript(definition: &LocalMappingDefinition) -> Result<String, String> {
    let mut output = String::from(
        "// Generated by SSDEV Desktop. Regenerate after mapping changes.\n\
import type { InvokeResponse, JsonObject, JsonValue, PluginInvoker } from '@bsoft/ssdev-web-bridge'\n\n",
    );
    let methods = typescript_method_plans(definition);
    for plan in &methods {
    ...
    for plan in methods {
        ...
        output.push_str(&format!(
            ...
            serde_json::to_string(&plan.service.service_id)
                .map_err(|error| error.to_string())?,
            serde_json::to_string(plan.request_name).map_err(|error| error.to_string())?,
        ));
    }
    output.push_str("}\n");
    Ok(output)
...
fn typescript_method_plans(definition: &LocalMappingDefinition) -> Vec<TypeScriptMethodPlan<'_>> {
    let mut simple_name_counts = HashMap::new();
    for service in &definition.services {
        for method in &service.methods {
            let request_name = method.alias.as_deref().unwrap_or(&method.name);
            *simple_name_counts
                .entry(ts_camel_identifier(request_name))
                .or_insert(0_usize) += 1;
        }
    }
    ...
    for service in &definition.services {
        for method in &service.methods {
            let request_name = method.alias.as_deref().unwrap_or(&method.name);
            let simple_name = ts_camel_identifier(request_name);
            ...
            if matches!(base_name.as_str(), "constructor" | "bridge") {
                base_name = format!("invoke{}", ts_pascal_identifier(&base_name));
            }
            let client_method = unique_typescript_identifier(base_name, &mut used_names);
            let stem = ts_pascal_identifier(&client_method);
...
mod tests {
    ...
    #[test]
    fn generated_typescript_uses_clean_names_bridge_types_and_public_routes() {
        let source = tempfile::tempdir().unwrap();
        let component = source.path().join("reader.bat");
        fs::write(&component, b"@echo off\r\n").unwrap();
    ...
    #[test]
    fn generated_typescript_defaults_empty_inputs_and_disambiguates_only_collisions() {
        let source = tempfile::tempdir().unwrap();
        let component = source.path().join("reader.bat");
        fs::write(&component, b"@echo off\r\n").unwrap();
        let mut definition = fixture_definition(&component);
        definition.services[0].methods[0].parameters.clear();

        let no_input = generate_typescript(&definition).unwrap();
        assert!(no_input.contains("read(parameters: ReadParameters = {})"));

        definition.services[0].methods[0].alias = Some("readCard".into());
        let public_alias = generate_typescript(&definition).unwrap();
        assert!(public_alias.contains("readCard(parameters: ReadCardParameters = {})"));
        assert!(public_alias
            .contains("invokePlugin<ReadCardData>(\"ReaderService\", \"readCard\", parameters)"));
        definition.services[0].methods[0].alias = None;

        let mut second_service = definition.services[0].clone();
        second_service.service_id = "WriterService".into();
        definition.services.push(second_service);
        let duplicate_routes = generate_typescript(&definition).unwrap();
        assert!(duplicate_routes
            .contains("readerServiceRead(parameters: ReaderServiceReadParameters = {})"));
        assert!(duplicate_routes
            .contains("writerServiceRead(parameters: WriterServiceReadParameters = {})"));

        let mut colliding_identifiers = fixture_definition(&component);
        colliding_identifiers.services[0].methods[0].name = "read-card".into();
        let mut second_method = colliding_identifiers.services[0].methods[0].clone();
        second_method.name = "read_card".into();
        colliding_identifiers.services[0]
            .methods
            .push(second_method);
        let sanitized = generate_typescript(&colliding_identifiers).unwrap();
        assert!(sanitized.contains("readerServiceReadCard(parameters:"));
        assert!(sanitized.contains("readerServiceReadCard2(parameters:"));

        colliding_identifiers.services[0].methods.truncate(1);
        colliding_identifiers.services[0].methods[0].name = "constructor".into();
        let constructor = generate_typescript(&colliding_identifiers).unwrap();
        assert!(constructor.contains("invokeConstructor(parameters:"));
        assert!(!constructor.contains("  constructor(parameters:"));

        colliding_identifiers.services[0].methods[0].name = "bridge".into();
        let bridge_member = generate_typescript(&colliding_identifiers).unwrap();
        assert!(bridge_member.contains("invokeBridge(parameters:"));
        assert!(!bridge_member.contains("  bridge(parameters:"));
    }
