...
use webplus_plugin_config::{
    build_local_mapping_integrity, generate_typescript_client, ParameterDefinition, PluginManifest,
...
fn generate_typescript(definition: &LocalMappingDefinition) -> Result<String, String> {
    generate_typescript_client(&definition.display_name, &definition.services)
        .map_err(|error| error.to_string())
}
