...
- 清单生成的 TypeScript 文件在兼容普通 `PluginInvoker` Client 之外提供独立的类型化 tracked API，让打印、写卡业务复用固定路由和输入/响应类型完成持久调用及状态查询，不再手抄 service/method；生成接口只接受由 SDK 创建或从业务存储中严格解析的规范 UUID v4，SDK 还把五种 tracked 状态分类为查询同一操作、处理响应、业务恢复或先对账，所有分类都明确禁止自动重放。
