...
- 清单生成的 TypeScript 文件在兼容普通 `PluginInvoker` Client 之外提供独立的类型化 tracked API，让打印、写卡业务复用固定路由和输入/响应类型完成持久调用及状态查询，不再手抄 service/method；生成接口只接受由 SDK 创建或从业务存储中严格解析的规范 UUID v4，并自动校验 Desktop 返回的五种状态、桥接响应外层和 JSON 数据。SDK 的统一 settlement 把直接调用 Promise 收敛为状态或脱敏失败，不向业务复制原异常；完成响应继续区分落盘已确认与未确认，后者要求业务处理响应同时记录恢复风险，任何结果都不会授权自动重放。
