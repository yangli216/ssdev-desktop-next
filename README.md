...
在占用 Windows 实机前，可先在任意开发平台运行 `ssdev-plugin-tool matrix-check --plugin-dir <prepare暂存目录> --matrix <定稿矩阵>`；多插件联合矩阵使用 `--plugin-root <插件根目录>`。封包后，单插件运行 `release-check --package ...`；多插件项目则用 [发布集合规范](docs/plugin-release-set.example.json) 运行 `release-set-check --spec ...`。两者都用实际包内清单联合验签并绑定包、信任库和矩阵摘要，与正式运行器共享 schema、路由、精确输入、占位符、复核标记和全方法覆盖规则，但不会加载组件或替代硬件验证。
