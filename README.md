...
- 清单生成的 TypeScript 文件在兼容普通 `PluginInvoker` Client 之外提供独立的类型化 tracked API，让打印、写卡业务复用固定路由和输入/响应类型完成持久调用及状态查询，不再手抄 service/method；生成接口只接受由 SDK 创建或从业务存储中严格解析的规范 UUID v4。五种成功返回状态和命令拒绝分别由 SDK 分类，后者使用版本化的 phase/code 对象而非本地化错误文本；任何结果都不会授权自动重放。
