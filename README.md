...
同 ID 签名插件的本地安装、仓库更新/回退、项目导入和管理员重扫还会对比当前公共 Web Bridge 契约；删除 route/alias、改变输入输出或增加必填输入会失败关闭，安全新增和需要原生复核的变化以聚合计数进入预览。本地动态映射保持开发期可编辑，不受发布兼容门禁限制。
上次受控激活的签名契约还会以无路径、无调用数据的有界本地基线保存；应用退出期间人工换入的同 ID 破坏性签名包会在下次启动被隔离，不能通过重启绕过兼容门禁。显式卸载成功后才清除对应基线墓碑。
