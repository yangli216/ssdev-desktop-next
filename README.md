...
生产策略使用 `ssdev-cutover-evidence prepare-policy` 自动生成：版本、提交和全部材料/插件/bundle/信任库摘要来自已复验试点输入与候选包，人工只批准证据时效、GO 现场执行窗口、迁移覆盖下限和四个职责签名人。schema 8 策略固定 QA 与最终审批信任库，并必须先由现有最终审批职责以独立 `cutover-policy` 域签名；`decide` 不接受未签名或被替换的策略。schema 3 GO 决策继续固定策略封套和实际两份信任库；同名 keyId 的替代公钥库不能通过判定或签发。归档历史复验继续使用 `verify-go`，真正部署前必须再运行 `check-current-go`，确保完整签名 GO 仍位于策略批准的最长七天执行窗口内，避免重放旧审批。工具在写入前二次复验输入且禁止覆盖，避免把复制错误或不匹配版本固化为看似有效的策略。
