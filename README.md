...
将项目配置、签名插件与本地映射迁移到目标机器的部署包流程见 [docs/project-bundles.md](docs/project-bundles.md)。
...
在占用 Windows 实机前，可先在任意开发平台运行 `ssdev-plugin-tool matrix-check --plugin-dir <prepare暂存目录> --matrix <定稿矩阵>`；多插件联合矩阵使用 `--plugin-root <插件根目录>`。封包后再运行 `ssdev-plugin-tool release-check --package <签名包> --trust-store <信任库> --matrix <定稿矩阵>`，用实际包内清单联合验签并绑定包、信任库和矩阵摘要。两者与正式运行器共享 schema、路由、精确输入、占位符、复核标记和全方法覆盖规则，但不会加载组件或替代硬件验证。
