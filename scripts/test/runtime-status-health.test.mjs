...
test('control console exposes stale status and blocks new business launches', async () => {
  ...
  assert.match(source, /runtimeStatusStale[^\n]+状态不可用/)
  assert.match(source, /runtimeStatusStale[^\n]+部署状态无法确认/)
  assert.match(source, /busy \|\| controlLoadFailed \|\| controlRefreshIncomplete \|\| runtimeStatusStale \|\| !snapshot\?\.config\.website/)
  assert.match(source, /桌面核心状态连续刷新失败/)
  assert.match(source, /ready: deploymentCheck\.ready && !controlLoadFailed && !controlRefreshIncomplete && !runtimeStatusStale/)
