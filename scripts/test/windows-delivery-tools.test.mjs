...
test('Windows delivery tools stay separate from the ordinary desktop installer', async () => {
  ...
  assert.match(buildScript, /"x64\/WebView2Loader\.dll"/)
  assert.match(buildScript, /webview2-com-sys/)
  assert.match(buildScript, /webView2Status: available/)
...
test('Windows delivery toolkit is source-bound, signed in production, and fully inventoried', async () => {
  ...
  assert.match(buildScript, /webView2LoaderAuthenticodeVerified = \$true/)
  assert.match(buildScript, /webView2LoaderPackageVersion/)
  assert.match(buildScript, /Get-FileHash -LiteralPath \$webView2LoaderDestination -Algorithm SHA256/)
  assert.match(buildScript, /Microsoft Corporation/)
...
test('delivery toolkit documentation keeps unsigned CI output out of production', async () => {
  ...
  assert.match(documentation, /`artifacts\.json` 覆盖.*全部可执行文件.*和 SBOM/)
  assert.match(documentation, /预装精确版本 `cargo-cyclonedx 0\.5\.9`/)
  assert.match(documentation, /ssdev-desktop-doctor\.exe inspect/)
  assert.match(documentation, /webView2Status/)
  assert.match(documentation, /`available`.*`unavailable`.*`probe-unavailable`/)
