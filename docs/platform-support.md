...
- Windows x64 另外构建不进入普通用户 NSIS 的项目交付工具包，包含离线启动诊断、试点、迁移、插件发布、签名、切换判定及正式黄金矩阵工具，并用 `release.json`、8 份 CycloneDX 1.5 SBOM 和 `artifacts.json` 记录源码版本、依赖图及完整文件集；生产分发另要求组织签名最终归档；
