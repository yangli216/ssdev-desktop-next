...
2. 在“项目配置 → 项目部署包”导出当前项目草稿。导出先确定性封装组件，再从这些待写入项目包的精确候选重新解包，检查联合服务路由、来源授权覆盖、磁盘与活动服务一致性，并为每种实际架构启动隔离宿主完成无业务副作用的预检；通过后显示整个项目包的 SHA-256，再由组织发布系统生成同目录 detached 签名封套。
