...
6. 以当前桌面配置和已验签来源策略检查安装后的全部签名插件路由；每条规范方法名及 alias 至少要被一个当前业务来源授权。未覆盖路由只返回数量和稳定诊断码，不暴露来源或路由名称，也不会进入宿主预检。
7. 在签名暂存目录中，为候选插件使用的每种 x86/x64 架构启动一次真实隔离宿主，完成认证管道、二次验签和 Health 往返后立即停止；预检不执行业务方法，也不停止当前健康宿主。失败时旧目录和旧路由完全不变。
8. 预检成功后进入目标插件自己的维护锁，只排空该插件的在途调用并停止它的活动宿主；其他插件继续服务。写入有界事务日志后原子切换目录，对目标目录再次验签并验证完整路由，失败则恢复旧目录。
9. 二次验签和路由验证成功后，只原子替换目标插件的路由；跨插件服务冲突在旧路由删除前失败。随后以原子重命名提交事务。崩溃后启动恢复器会回滚未提交事务，并清理已提交或废弃的暂存目录。
