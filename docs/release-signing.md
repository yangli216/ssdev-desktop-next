...
- 三类执行证据分别检查自身严格 schema、来源提交、执行环境、输入摘要、覆盖与结果一致性，并使用不同域分隔 payload；插件矩阵 schema 2 的审核摘要额外公开 `packageSetSha256`，把硬件结论绑定到批准的确定性包集合；Windows 包 schema 2 的审核摘要公开最终产物清单、实际安装插件信任库及 x86/x64 宿主 SHA-256，使审批者能看到最终 Go/No-Go 用于锁定候选包并和硬件矩阵对账的制品身份。它们共享 `cutover-evidence` 用途，但生产策略可为每类证据指定不同 `keyId` 以隔离 QA 职责。
