...
命令行摘要与 JSON 报告使用同一组已经排序的稳定阻断码。材料不齐时会逐行输出 `blocker: <code>` 并提醒修正后使用新的报告路径再次执行，因为原报告不会覆盖；CI 和实施人员无需先猜测哪一类材料缺失。材料齐全时不再输出文件名、目录或项目标签，只输出 `material set sha256`，并明确下一步是把同一材料根、manifest 和报告交给接收方独立执行 `verify`。该摘要便于现场处理，但不能替代 JSON 报告的归档和复验。
...
复验完整时，控制台摘要会再次输出同一个材料集合 SHA-256，并把唯一下一步指向由该已复验集合派生的正式迁移审计；复验到一份真实但不完整的旧报告时，则列出报告自身阻断码并要求修正材料后重新生成不可覆盖的新报告，不会把“报告未被篡改”误报成“材料已经齐全”。
