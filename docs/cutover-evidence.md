...
上述入口来自独立 Windows 项目交付工具包，会先复验工具包完整清单并使用相邻 EXE，不要求目标验证机安装 Rust。若在源码构建机直接执行，可继续使用 `scripts/test-windows-package.ps1`；两种入口运行同一验收逻辑且都必须绑定候选对应的干净源码工作区。工具包模式进入验收流程后的失败只输出稳定 `blocker`、固定 `action` 和 `evidence: not produced`；成功输出 `windows package: CLEAR`，仍需完成本节的 QA 签名与最终判定。

工具只接受 schema 1、`unsigned-local-record`、Windows、候选版本一致、14 个固定检查项齐全且 `project-identity`、`business-frontend` 均为 `pass` 的深度记录；记录必须在包证据生成前一小时内产生。它会在读取前后复算摘要并把摘要及生成时间写入待签名的 schema 7 包证据，原始记录仍应随 QA 归档保存。未传参数时脚本显式写入空绑定，适用于普通 CI，但最终判定不会把它当作现场验收。
...
候选 bundle 必须是构建产出的完整根目录，不能只传 NSIS 安装器。命令在现行授权检查前后分别按 `metadata/artifacts.json` 对全部受清单覆盖文件重新计算大小和 SHA-256，并要求 `release.json`、产物清单、应用版本和源码提交与签名 Windows 证据及 GO 精确一致；清单还必须恰好覆盖一个顶层 `nsis/*-setup.exe`。完整但属于另一构建的 bundle、单独替换的安装器、缺失/额外文件、零个/多个 NSIS 或损坏清单都会输出稳定 `cutover-candidate-bundle-mismatch` 并返回 `3`；验证期间发生变化返回 `1`。`CURRENT-GO` 会输出获准产物清单摘要和 `installer: nsis/...-setup.exe` 相对路径，实施人员必须立即从同一个受保护根目录启动这一文件，不能在检查后复制、重新打包或换用同名文件。命令仍不自动安装，因此其授权不会覆盖检查结束后的人工替换。
