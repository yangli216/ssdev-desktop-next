...
然后从 clean 候选源码、已复验试点三件套、候选 Windows bundle、QA 证据信任库和人工批准输入生成不可覆盖的 schema 7 策略：
