...
3. 检查更新时先用目标 Desktop SemVer 重新发现全部签名插件；任何插件缺少明确兼容范围、范围不匹配或完整性异常都会作为阻塞项展示，不提供安装操作。
4. 可安装结果生成只用于本次确认的域分隔计划标识，绑定当前版本、目标版本、发布日期、目标平台、下载 URL、Minisign 签名、页面展示的发布说明，以及当前全部签名插件的身份、签名 keyId 和完整文件载荷。开始新一轮检查时会先清除旧 pending，检查失败不能继续安装上一轮对象。
5. 用户确认安装时必须提交计划标识；客户端在插件安装锁内重新恢复事务、读取精确 pending 更新并重建插件集合摘要。pending 被另一轮检查替换，或插件被安装、更新、卸载及内容发生变化时，旧确认在下载前失败。
6. 更新包流式写入用户本地数据目录，最多 256 MiB；重定向和最终下载均受 HTTPS-only 客户端约束。
7. 下载过程中使用现代预哈希 Minisign 签名做流式验证，未通过验签不会进入安装阶段。验签并完整读取后再次验证目标 Desktop 兼容性和插件集合摘要，阻止下载期间发生的插件漂移。
...
- 发布 Desktop 前必须先保证每个仍需保留的签名插件已发布覆盖目标版本的兼容范围；客户端会阻止导致现有插件被隔离的升级，不以“升级后再修插件”作为正常流程。
