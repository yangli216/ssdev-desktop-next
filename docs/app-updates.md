...
4. 用户确认安装时，在插件安装锁内再次恢复事务并执行同一兼容性检查，防止检查后插件集合变化绕过门禁。
5. 更新包流式写入用户本地数据目录，最多 256 MiB；重定向和最终下载均受 HTTPS-only 客户端约束。
6. 下载过程中使用现代预哈希 Minisign 签名做流式验证，未通过验签不会进入安装阶段。
7. 验签成功后关闭业务窗口并停止所有 DLL/COM 插件宿主，再启动系统安装程序。
8. Windows 安装器会退出当前客户端；macOS/Linux 安装完成后由客户端重启。
