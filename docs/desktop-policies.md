...
配置页可以新增、删除、启停快捷键，并从上述五种固定动作中选择。组合键必须是 1–64 个无空格可打印 ASCII 字符，配置限制为 32 项，启用的快捷键忽略大小写后不能重复；停用项同样不能保存空白或控制字符，避免把无效草稿带入项目包。页面只做确定性格式和重复检查，最终是否被 Windows 或其他软件占用由系统注册结果决定。

`capture-region` 打开只具备截图 IPC 的本地全屏遮罩，用户确认后只向原业务窗口发送裁剪后的 PNG。保存配置时会先尝试注册完整的新集合；任一项失败时恢复旧集合，配置文件也不会被替换。快捷键变更不关闭已经打开的业务页面。旧 `keymap.json` 中的 `snippet` 和 `eval(...)` 不会迁移。
